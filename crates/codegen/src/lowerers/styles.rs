...
impl<'a> LoweringState<'a> {
    pub(crate) fn collect_style_properties<'b>(
        &self,
        statements: &'b [HirStyleStatement],
    ) -> Vec<&'b StylesDefinition> {
    ...
    fn compute_property_prim_counts(&self, properties: &[ResolvedProperty]) -> Vec<usize> {
        properties
            .iter()
            .map(|rp| self.hir.flatten_type(rp.hir_type).len())
            .collect()
    }
    ...
    pub(crate) fn lower_stylesheet(
        ...
        self.create_style_constructor(id, decl, struct_ty, usages, &resolved, ir)?;
        self.create_style_apply_function(id, struct_ty, &resolved, ir)?;

        Ok(())
    }

    pub(crate) fn resolve_style_inheritance<'b>(
        &self,
        usages: &[HirStyleUsage],
        own_props: &[&'b StylesDefinition],
    ) -> Vec<ResolvedProperty<'b>> {
        let mut resolved: Vec<ResolvedProperty<'b>> = Vec::new();
        let mut seen_codes: HashSet<StyleProperty> = HashSet::new();

        for (usage_idx, usage) in usages.iter().enumerate() {
            let file = self.hir.get_file(usage.style.file_id);
            ...
            for def in &parent_props {
                let name_str = self.hir.get_name(def.name);
    ...
    fn populate_style_struct_fields(
        ...
        let field_types: Vec<IRTypeId> = properties
            .iter()
            .flat_map(|rp| self.hir.flatten_type(rp.hir_type))
            .map(|prim_ty| self.types.get_or_create_ir_type(prim_ty, ir))
    ...
    fn flatten_struct_value(
        ...
        let mut map_types = |tys: &[DedupPoolId<HirType>]| {
            ...
            for (i, field_ty) in tys.iter().enumerate() {
                ...
                result.extend(self.flatten_struct_value(field_val, *field_ty, ctx)?);
            }
            Ok(result)
        };
        match view.raw() {
            ...
            HirType::Reference { rf, .. } => self.flatten_struct_value(value, *rf, ctx),
            _ => Ok(vec![value]),
        }
    }
    ...
    fn create_style_constructor(
        ...
        let hir_type_args = if let Some(viewer) = self.hir.view(decl.ty).is_style() {
            viewer.args().to_vec()
        } else {
            Vec::new()
        };
        ...
        let arg_ir_types: Vec<IRTypeId> = hir_type_args
            .iter()
            .map(|a| self.types.get_or_create_ir_type(*a, ir))
            .collect::<Result<Vec<_>, _>>()?;
        ...
        for statement in statements {
            if let HirStyleStatement::Statement(s) = statement {
                self.lower_statement(*s, &mut ctx)?;
            }
        }
        ...
        for &usage_idx in &needed_usages {
            ...
            let param_values = self.get_usage_args(usage, &mut ctx)?;
            let parent_data = &self.styles[&usage.style];
            let struct_value = ctx.call(parent_data.init_func, &param_values, parent_data.struct_ty);
        ...
        for rp in properties {
            let value = match &rp.source {
                PropertySource::Own(def) => self.lower_expression(def.expr, &mut ctx)?,
            ...
            let primitives = self.flatten_struct_value(value, rp.hir_type, &mut ctx)?;
    ...
    fn create_style_apply_function(
        ...
        let prim_counts = self.compute_property_prim_counts(properties);
