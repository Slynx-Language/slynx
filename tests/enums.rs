...
#[test]
fn enum_examples() {
    let dir = PathBuf::from("examples/enums");
    let mut entries: Vec<_> = std::fs::read_dir(&dir)
        .expect("enums examples directory should exist at examples/enums/")
        .filter_map(|e| e.ok())
    ...
    assert!(
        !entries.is_empty(),
        "no .slx files found in examples/enums/"
    );
